...
if (isset($_POST['submit'])) {
	$puname = filter_var($_POST['username'], FILTER_VALIDATE_EMAIL);
	$ppwd = $_POST['password'];

	if ($puname && $ppwd) { // Validate email and password
		...
		if ($user) {
			// First, attempt to verify using `password_verify`
			if (password_verify($ppwd, $user['password'])) {
				...
				$log_stmt->bind_param("issi", $pid, $puname, $uip, $status);
				$log_stmt->execute();
				$log_stmt->close();

				header("location:dashboard.php");
			} elseif ($user['password'] === md5($ppwd)) {
				// If password matches old `md5` hash, rehash it
				$newPasswordHash = password_hash($ppwd, PASSWORD_DEFAULT);
